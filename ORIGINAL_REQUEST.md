...
## Follow-up — 2026-09-16T06:31:09Z

로컬 PC 내 다중 공공·행정 사업 추진 과정에서 무질서하게 누적된 업무 파일(기안서, 계획서, 예산서, 결과보고서 등)을 공공 표준 분류 체계로 자동 분류·아카이빙하고, 원본 무결성 보장(Dry-run) 및 100% 롤백이 가능한 안전한 파일 관리 파이프라인 도구를 구축한다.

Working directory: d:/Desktop/PORTFOLIO/PORTFOLIO - VITAL/tools/file_organizer
Integrity mode: development

## Requirements

### R1. 지능형 파일 스캔 및 공공 행정 표준 분류 엔진
- 대상 디렉토리(바탕화면, 다운로드, 문서 등 로컬 작업 영역) 내 다양한 형식(HWP/HWPX, PDF, XLSX, DOCX, TXT 등)의 파일 목록 및 메타데이터(생성일, 수정일, 파일명 패턴, 문서 본문 키워드)를 수집·분석할 것.
- 공공기관 표준 4단계 위계 체계(`[연도별] > [사업/프로젝트별] > [업무단계별(기획·품의 / 예산·지출 / 집행·행사 / 결과보고·정산)] > [문서유형]`)에 따라 파일을 고유 규칙 및 텍스트 휴리스틱으로 정확하게 분류할 것.
- 버전 파편화 파일(예: `_최종`, `_수정본`, `_v2`, `_최최종`) 및 중복 파일을 감지하여 식별 정보를 메타데이터에 기록할 것.

### R2. 사전 시뮬레이션(Dry-Run) 및 시각적 검토 리포트
- 실제 파일 이동 전, 원본 파일 시스템에 일체의 변경을 가하지 않는 Dry-Run 시뮬레이션 모드를 기본으로 제공할 것.
- 파일 이동 전후 경로, 분류 근거, 예상 이동 경로, 중복/버전 파일 현황을 명확히 대조할 수 있는 상세 검토 보고서(JSON 및 Markdown/HTML)를 자동 생성할 것.

### R3. 원본 보존 보장 및 양방향 무결성 롤백 엔진
- 사용자의 명시적 승인 전까지 원본 파일은 삭제되거나 유실되지 않아야 하며, 파일 이동 실행 시 모든 작업 내역을 트랜잭션 로그(Audit Log)에 기록할 것.
- 실행 후 사용자가 언제든 즉시 원상 복구할 수 있는 독립 롤백 스크립트(`rollback.py` 또는 원클릭 스크립트)를 자동 생성하고, 롤백 실행 시 파일 해시(SHA-256) 및 원위치가 완벽히 복원되도록 보장할 것.

### R4. 가상 모의 테스트 환경 및 자동 회귀 검증 스위트
- 실제 사용자 파일 시스템 적용에 앞서, 공공 행정 문서 양식(기획안, 지출결의서, 결과보고서, 행사계획 등)을 모사한 50개 이상의 다양한 가상 테스트 파일셋을 생성하는 벤치마크 생성기를 포함할 것.
- 파이프라인의 스캔, 분류 정확도, Dry-Run 불변성, 이동 정확도, 롤백 무결성을 외부 심사관(Auditor) 관점에서 독립 검증하는 자동화 테스트 스위트를 구비할 것.

## Acceptance Criteria

### 분류 정확도 및 스캔 무결성
- [ ] 50개 이상의 모의 공공 행정 파일셋에 대해 스캔 누락률 0%를 달성할 것.
- [ ] 연도, 사업명, 업무 단계(기획/예산/집행/결과) 3단계 이상 분류율이 90% 이상일 것.
- [ ] 중복 파일 및 버전 접미사(`_최종`, `_v2` 등)가 부여된 파일을 정확히 그룹화할 것.

### 안전성 및 Dry-Run 불변성
- [ ] Dry-Run 모드 실행 시 원본 디렉토리의 모든 파일 체크섬(SHA-256) 및 디렉토리 구조 변경이 0건(불변)임을 프로그램으로 입증할 것.
- [ ] Dry-Run 결과로 생성된 보고서에 모든 대상 파일의 원본 경로, 목표 분류 경로, 판정 사유가 누락 없이 기록될 것.

### 롤백 및 데이터 무손실 보장
- [ ] 실제 마이그레이션 실행 후 롤백을 수행했을 때, 100%의 파일이 원래 디렉토리 및 원래 파일명으로 복원될 것.
- [ ] 롤백 전후 모든 복원 파일의 SHA-256 체크섬이 최초 원본과 100% 일치할 것.
- [ ] 이동 실패 또는 중단 상황 발생 시 자동 롤백 및 안전 종료가 보장될 것.

### 독립 검증 자동화
- [ ] 단일 명령(예: `pytest` 또는 `npm test` 등)으로 모의 데이터 생성, 스캔, 분류, Dry-run 검증, 이동, 롤백 무결성 전 과정의 테스트를 자동 완주하고 종료 코드 0(Exit Code 0)으로 통과할 것.
