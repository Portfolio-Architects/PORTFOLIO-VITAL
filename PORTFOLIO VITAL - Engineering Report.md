...
* **개요 및 개발 목적**:
  - 건강페스티벌 추진 현황판 프론트엔드 및 데이터 파이프라인 전면 동기화:
    ...
    4. **Cloudflare Pages 24/7 Read-Only Replica 듀얼 싱크 완료**:
       - `node scripts/sync-festival-to-cloud.js`를 통해 Cloudflare KV 엔드포인트(`https://portfolio-hchps.pages.dev/api/festival/yangjae`)로 17개 부스 및 최신 일정 원격 동기화 배포 완료.
...
- [x] **2026 양재천 건강페스티벌 보건소 부서 부스 세분화, 카테고리 필터 '기타' 신설 및 Cloudflare Pages 24/7 레플리카 듀얼 싱크 (Milestone 148 - 2026-09-11)**
  - 사용자 요구사항: "건강페스티벌 보건소 부서별 부스 현황 최신화 및 대시보드 카테고리 필터 확장, Cloudflare Pages 동기화 배포"
  - 개선 내역:
    * **로컬 SSOT 최신화 및 보건소 부서 부스 개편 반영 (`data/FESTIVAL_YANGJAE_2026.json`)**:
      - 추진과제 1: 9. 11. 신규 걷기 코스 2km 답사 완료 및 9. 18. 전 부서 알림 일정 최신화.
      - 추진과제 2: 공원녹지과 포이공원 장소 및 전기 사용 협조 완료 이동 정리.
      - 추진과제 3 & 4: 의약과 약무팀·의무1팀 부스 신청서 접수, 9. 21. 온라인 사전접수 시스템 오픈 일정 조율 반영.
      - 부스 현황 개편: '의약과 의무1팀(CPR 체험존)', '질병관리과 감염병관리팀(강남도감)', '질병관리과 만성질환관리팀', '건강관리과(웰에이징)'로 정밀 세분화하고 보건행정과 총괄 운영·응급의료·VIP 대기 부스(4동) 확정 반영 (총 17개 주체, 25개 부스 규모 재편).
    * **대시보드 카테고리 필터 '기타' 탭 신설 및 UI 최적화 (`YangjaeFestivalDashboard.tsx`, `pages-template.html`)**:
      - `FESTIVAL_CATEGORIES`에 `'기타'` 탭 추가(`['전체', '민간', '보건소 부서', '기타']`).
    * **Cloudflare Pages 24/7 Read-Only Replica 듀얼 싱크 완료**:
      - KV 엔드포인트 원격 동기화 배포 및 `out/` 정적 번들 최신화.
  - 정량적 검증 성과:
    * 전체 Jest 회귀 테스트 (`npm test`): **32/32 Suites, 296/296 Tests ALL PASS (100%)**.

- [x] **재귀적 자기개선 - 공문서 및 행정 보고 표준 문체 헌장(Rule M) 정립 및 에이전트 행동 수칙 전면 동기화 (Milestone 149 - 2026-09-14)**
  - 사용자 요구사항: "엔지니어링 리포트하고 에이전트 md 파일 재귀적 자기개선 요청하는데, 제일 문제가 답변할때, 공문서 말투 및 문체를 잘 반영하지 못한다는점임"
  - 개선 내역:
    * **에이전트 행동 수칙 내 Rule M (공문서 및 행정 보고 표준 문체 헌장) 신설 (`AGENTS.md`)**:
      - 대한민국 공공기관(보건소·구청·시청) 실무관/사무관 수준의 공문서 표준 개조식 문체 전면 의무 적용.
      - 친근감 표명, 감정적 수식어, 구어체 및 사족 종결어미("~하겠습니다", "~인 것 같습니다" 등) 영구 배제.
      - 결론 및 실적을 최상단에 전진 배치하는 두괄식 행정 보고 원칙 확립.
    * **개조식 종결어미 체계 강제화**:
      - 실행 완료(`-조치 완료함`, `-반영함`, `-구현함`, `-동기화 완료함`), 계획(`-추진 예정임`), 상태(`-으로 확인됨`, `-체계를 유지함`) 등 명사형 및 개조식 종결어미 표준화.
    * **다단계 항목 기호 및 공문서 위계·띄어쓰기 규격화**:
      - 법정 공문서 위계(`1.` $\to$ `가.` $\to$ `1)` $\to$ `가)` $\to$ `⑴` $\to$ `㈎` $\to$ `①` $\to$ `㉮`) 및 보고서 딩뱃 기호(`󰏚` $\to$ `▢` $\to$ `❍` $\to$ `-` $\to$ `•` $\to$ `※`) 정립.
      - 항목 기호 뒤 1타(1칸) 띄우기 및 하위 항목 2타(2칸) 들여쓰기 원칙 명문화.
    * **행정 문장부호 및 단위 표기 표준 규격화**:
      - 연월일 마침표 표기(`2026. 9. 14.`), 24시각제 시간(`14:00`, `09:30~12:20`), 금액 병기(`금10,000,000원(금일천만원)`), 낫표(법령·사업 홑낫표 `｢ ｣`, 도서·간행물 겹낫표 `『 』`), 본문/붙임 끝 2타 띄기 `  끝.` 엄수.
    * **행정 어휘 순화 및 자가 점검 필터(Self-Verification Guard) 탑재**:
      - 중복 어휘("2월달", "기간 동안", "미리 예측", "새로 신설", "반드시 필요") 사용 금지.
      - 사용자 응답 출력 직전 공문서 개조식 어조 부합 여부를 사전 검증하는 자기 점검 가드 구축.
    * **마일스톤 및 에이전트 매니페스트 재귀적 자동 동기화**:
      - `node scripts/sync-rules.js` 구동을 통한 마일스톤 로그 최신화 완료.

- [x] **공문서 및 행정 표준 문체 내재화 10회 연속 재귀적 자기개선 (Milestones 150~159 - 2026-09-14)**
  - 사용자 요구사항: "재귀적 자기개선 10회 진행해보자 동일한 안건으로"
  - 10회 연속 개선 내역:
    * **Iteration 1 (Milestone 150)**: LLM 챗 라우트(`src/app/llm/chat/route.ts`) 시스템 프롬프트에 Rule M 개조식 종결어미, 두괄식 원칙, 일자/시간/금액 표기 프로토콜 전면 주입.
    * **Iteration 2 (Milestone 151)**: 프론트엔드 AI 비서 모달(`AIAssistantModal.tsx`) 타이틀을 '행정 인텔리전스 어시스턴트'로 개편하고 예시 질의를 공공기관 업무 보고 양식으로 교정.
    * **Iteration 3 (Milestone 152)**: 대한민국 공문서 표준 포매터 및 린터 유틸리티(`src/lib/administrativeFormatter.ts`) 신규 개발 (일자, 24시각제, 금액 한글 병기, 34개 중첩어 정제, 18개 개조식 종결어미 변환).
    * **Iteration 4 (Milestone 153)**: 행정 포매터 단위 테스트 스위트(`__tests__/administrative-formatter.test.ts`) 9종 신규 구현 및 100% 통과(9/9 Tests ALL PASS).
    * **Iteration 5 (Milestone 154)**: LLM 챗 백엔드 `cleanGemmaResponse` 정제 파이프라인에 `sanitizeAdministrativeText` 사후 필터를 결합하여 모델 출력 실시간 교정.
    * **Iteration 6 (Milestone 155)**: 다중 에이전트 하네스 Evaluator(`src/lib/agents/evaluator.ts`)에 `evaluateAdministrativeText`를 탑재하여 문체 위반 시 자가 치유 피드백 제공.
    * **Iteration 7 (Milestone 156)**: `AGENTS.md` Rule M에 공문서 3단 구성(두문·본문·결문) 및 법정 수신자란(내부결재, 기관장/보조기관, 민원 회신) 표기 규정 심화 명문화.
    * **Iteration 8 (Milestone 157)**: `AGENTS.md` Rule M에 행정안전부 준거 20대 행정용어 순화 사전 및 다단계 법정 기호 대조표 공식 수록.
    * **Iteration 9 (Milestone 158)**: 엔지니어링 마일스톤 및 리포트 10단계 연쇄 반영 및 `node scripts/sync-rules.js` 자동 동기화 도구 파이프라인 구동.
    * **Iteration 10 (Milestone 159)**: 전체 Jest 회귀 테스트 스위트(33개 스위트, 305개 테스트) 구동 및 100% 무결성 검증 완료.

- [x] **제8회 강남구청장배 걷기대회 2.0km 양재천 코스도 밀미리교 벡터 방향 화살표 안티에일리어싱 정밀 보정 및 고시인성 HY견고딕 타이틀 최종 렌더링 릴리즈 (Milestone 160 - 2026-09-14)**
  - 사용자 요구사항: "밀미리교는 이름이 두번 나오는데, 한번만 나오게 수정해주고, 강남구청장배 걷기대회 코스 폰트는 더 가독성 높은것으로 수정해줘", "화살표 깨진것 수정 가능한가? 밀미리교 쪽에"
  - 세부 조치 및 엔지니어링 내역:
    * **밀미리교 방향 화살표 서브픽셀 안티에일리어싱(Anti-Aliasing) 교체**:
      - 기존 PIL 폴리곤 직접 드로잉으로 인해 발생하던 계단 현상(Aliasing/픽셀 깨짐) 및 하단 잔류 고스트 아웃라인을 완전 제거함.
      - 영동4교의 고품질 벡터 상향 화살표 에셋을 수직 반전(FLIP_TOP_BOTTOM)하여 백색 스트로크 외곽선과 청록색 내부 채움이 융합된 무결점 하향 방향 화살표로 재합성함.
      - 밀미리교 교각 상판(`y=308..488`)과 하천 수면 경계를 규격화하여 완전 정렬하고, 북측 상부 산책로 진입 곡선(`y=306, x=145.5`) 및 하부 완주 지점 곡선(`y=485, x=145.5`) 간 0-픽셀 오차 없는 연속 수직 경로를 구축 완료함.
    * **다리 명칭 단일화 및 공공기관 표준 HY견고딕 타이틀 반영**:
      - 교각 기둥부 수직 텍스트 잔여물을 소거하고 양재천 수면 위 단일 수평 라벨("밀미리교") 체계로 확립함.
      - 타이틀을 "제8회 강남구청장배 걷기대회 코스"로 변경하고 윈도우 표준 고시인성 공문서 서체인 HY견고딕(`H2GTRE.TTF`, 74pt) 2톤 컬러링을 적용 완료함.
    * **최종 산출물 자동 배포 및 동기화**:
      - 바탕화면 대상 경로(`D:\Desktop\2026 양재천 걷자! 건강 페스티벌\양재천 사진\제8회_강남구청장배_걷기대회_코스도_최종.jpg`) 및 브레인 저장소 듀얼 저장 완료함.
      - `node scripts/sync-rules.js` 자동 실행을 통한 `AGENTS.md` 최신 마일스톤 로그 동기화 완료함.

- [x] **제8회 강남구청장배 걷기대회 코스도 메인 타이틀 '(2km)' 코스 거리 공식 명기 및 고시인성 HY견고딕 2톤 렌더링 릴리즈 (Milestone 161 - 2026-09-15)**
  - 사용자 요구사항: "걷기 코스 타이틀 옆에 "(2km)" 코스 길이를 넣어줘"
  - 세부 조치 및 엔지니어링 내역:
    * **메인 헤드라인 코스 거리 '(2km)' 추가 및 2톤 컬러링 적용**:
      - 기존 타이틀 "제8회 강남구청장배 걷기대회 코스" 우측에 대회 규격 거리 "(2km)"를 공식 추가하여 『제8회 강남구청장배 걷기대회 코스 (2km)』로 확정함.
      - 윈도우 공문서 표준 고시인성 서체인 HY견고딕(`H2GTRE.TTF`, 66pt)을 적용하여 좌우 여백(각 57px)과 시각적 밸런스를 확보함.
      - '강남구청장배'와 '(2km)'를 통일된 딥 틸(청록, `#1a7685`) 포인트 컬러로 대칭 배색하여 대회의 대표성과 코스 길이를 한눈에 식별 가능하도록 시인성을 극대화함.
    * **최종 산출물 자동 배포 및 동기화**:
      - 바탕화면 대상 경로(`D:\Desktop\2026 양재천 걷자! 건강 페스티벌\양재천 사진\제8회_강남구청장배_걷기대회_코스도_최종.jpg`) 및 브레인 저장소 듀얼 저장 완료함.
      - `node scripts/sync-rules.js` 자동 실행을 통한 `AGENTS.md` 최신 마일스톤 로그 동기화 완료함.
- [x] **제8회 강남구청장배 걷기대회 2.0km 코스도 주로 방향 전면 역전 (북측 상부 산책로 가는 길 ↔ 남측 상부 산책로 오는 길) 및 영동3교 (1.5km) 배지·보행교 벡터 화살표 완벽 전환 (Milestone 162 - 2026-09-16)**
  - 사용자 요구사항: "코스 주로 방향이 반대로 바뀌었어 북측 상부 산책로가 가는길이고 남측 상부산책로가 오는길이야"
  - 주요 조치 및 엔지니어링 내역:
    * **주로 방향 텍스트 및 산책로 구간 화살표 전면 역전**:
      - 북측 상부 산책로 텍스트를 기존 '(오는 길)'에서 '북측 상부 산책로 (가는 길)'로 전면 전환하고, HY견고딕(30pt) 정밀 서체 렌더링을 적용함.
      - 남측 상부 산책로 텍스트를 기존 '(가는 길)'에서 '남측 상부 산책로 (오는 길)'로 전면 전환하고, HY견고딕(24pt) 정밀 서체 렌더링을 적용함.
      - 북측 산책로 구간 화살표를 서향(◀)에서 동향(우측 ▶)으로 수평 반전(FLIP_LEFT_RIGHT)하여 100% 벡터 안티에일리어싱 품질로 합성함.
      - 남측 산책로 구간 화살표 2개소를 동향(▶)에서 서향(좌측 ◀)으로 수평 반전(FLIP_LEFT_RIGHT)하여 100% 벡터 안티에일리어싱 품질로 합성함.
    * **보행교 횡단 방향 화살표 재배치 (밀미리교 상향 ▲, 영동4교 하향 ▼)**:
      - 밀미리교(출발 후 북측 진입교): 출발지(수변문화쉼터)에서 북측 산책로로 진입하는 동선에 맞춰 상향(북측 ▲) 고품질 벡터 화살표로 재배치함.
      - 영동4교(1.0km 반환교): 북측 산책로에서 남측 산책로로 복귀하는 동선에 맞춰 하향(남측 ▼) 고품질 벡터 화살표로 재배치함.
      - 교각 상판 슬라이스(y=325..345) 수직 타일링 기법을 적용하여 이음새(Seam)와 경계 사각형 아티팩트를 0-픽셀로 완전 소거함.
    * **영동3교 중간 배지 누적 거리 정합성 갱신 ('(0.5km)' -> '(1.5km)')**:
      - 주로 방향 역전에 따라 남측 귀로(오는 길) 상의 영동3교 통과 시점 누적 거리를 반환점(1.0km) 이후 진행 거리에 맞추어 '(1.5km)'로 정밀 갱신함 (0km 출발 -> 1.0km 영동4교 반환 -> 1.5km 영동3교 통과 -> 2.0km 완주).
    * **최종 산출물 자동 배포 및 동기화**:
      - 바탕화면 실무 파일(`D:\Desktop\2026 양재천 걷자! 건강 페스티벌\양재천 사진\제8회_강남구청장배_걷기대회_코스도_최종.jpg`) 및 브레인 디렉토리에 동시 저장 완료함.
      - `node scripts/sync-rules.js` 자동 실행을 통해 `AGENTS.md` 최신 마일스톤 로그 동기화 완료함.

- [x] **강남구 '손목닥터9988' 걷기 챌린지 16:9 공공 인포그래픽 포스터 신규 시안 확정 및 표준 디자인 프레임워크 정립 (Milestone 163 - 2026-09-16)**
  - 사용자 요구사항: "글씨 수는 최소화 해서.. 인포그래픽이니까, PPT 자료처럼 만들필요는 없어", "강남구 로고는 빼버리고 슬로건만 우상단에 표시하고, 하단에 마커랑 텍스트 정렬이 안맞잖아", "아예 새로 디자인 해봐 다른 시안으로", "굿, 시안 좋다, 이 내용 엔지니어링 리포트에 기록하자, 다음에 다른 인포그래픽 뽑을 때 비슷한 시안 나올수 있게"
  - 세부 조치 및 엔지니어링 내역:
    * **16:9 공공 캠페인 인포그래픽 포스터 표준 프레임워크 수립**:
      - **상단 3분할 밸런스 레이아웃**: 좌측 볼드 헤드라인('손목닥터9988 걷기 챌린지 (9.16~12.31)'), 중앙 3대 컬러 캡슐(선착순 4,500명, 총 900만 포인트, 회당 2,000P 서울페이), 우측 상단 공식 3단 붓글씨 슬로건(`강남을 힘차게! / 구민을 신나게! / 강남 대전환!!`) 독립 배치.
      - **CI 로고 배제 원칙 확립**: 불필요한 시각 간섭과 중복을 차단하기 위해 지자체 로고를 완전 배제(NO Logo)하여 개방감과 트렌디한 포스터 미학을 극대화함.
      - **텍스트 최소화 및 비주얼 중심화**: PPT식 장황한 설명 줄글을 100% 제거하고, [핵심 수치] + [장소 키워드] + [풍성한 일러스트레이션] + [리워드 뱃지] 중심의 순수 인포그래픽 조판 규격 확립.
    * **5대 릴레이 코스 수직 그리드 0.1px 칼정렬 (Strict Vertical Alignment)**:
      - 5개 컬럼의 중심축($X_1=165, X_2=429, X_3=692, X_4=954, X_5=1213$)을 수학적으로 산출 및 고정.
      - [상단 원형 마커 1~5] - [코스 타이틀] - [랜드마크 일러스트(기와궁궐, 축제무대, 벚꽃수변, 명산등산, 전통시장)] - [하단 선착순 인원 및 2,000P 뱃지]의 세로 중심을 0-오차로 완벽 일치시킴.
    * **오탈자 전수 교정 및 공공 표준 명칭 안착**:
      - 기존 AI 생성 오류('3산 트로케킹', '3개 산', 'Okt', 'Deg')를 전면 일소하고, 정식 공공 행정 용어인 ｢3대 명산 트레킹｣ 및 표준 회차·월차(`9월`, `10월 ①`, `10월 ②`, `11월`, `12월`)로 정제 완료함.
    * **자산 영속화 및 규칙 매니페스트 동기화**:
      - 고해상도 최종 산출물(`gangnam_walking_poster_concept2.jpg`, `gangnam_walking_poster_concept2.png`) 영속화.
      - `AGENTS.md`에 규칙 N(16:9 공공 인포그래픽 포스터 표준 디자인 헌장)을 신설하여 향후 동일 품질 인포그래픽 즉각 재생산 보장.
      - `node scripts/sync-rules.js` 파이프라인 자동 실행 완료.

- [x] **공공 행정 업무 파일 자동 정리 및 보존 아카이빙 파이프라인 도구 (tools/file_organizer) 릴리즈 (Milestone 162 - 2026-09-16)**
  - 사용자 요구사항: "M4: Comprehensive Regression Testbed & Real Document Generator"
  - 세부 조치 및 엔지니어링 내역:
    * **60개 공공 행정 실물 모의 문서 생성 벤치마크 테스트베드 구축 (`tools/file_organizer/benchmark/test_generator.py`)**:
      - 4대 행정 업무 단계(기획 15건, 예산 15건, 집행 15건, 성과 15건) 균등 분할 생성.
      - 6대 공공 행정 표준 포맷(HWPX, HWP, PDF, XLSX, DOCX, TXT) 정밀 생성기 구현.
        - HWPX: OWPML XML 표준 헤더, 섹션, 메타데이터 패키징.
        - HWP: 4096바이트 섹터 기반 표준 OLE Compound 구조 및 `PrvText` 스트림 실물 바이너리 생성.
        - PDF: PyMuPDF(fitz) 기반 '맑은 고딕' 렌더링 및 행정 딩뱃 기호(`󰏚`, `▢`, `❍`, `-`) 표기.
        - XLSX: openpyxl 기반 산출내역서/지출결의서 서식 및 행정 회계 표 조판.
        - DOCX: OOXML 기반 공문서 서식 및 본문/붙임 개조식 서식 구성.
        - TXT: UTF-8 및 CP949 인코딩 공문 텍스트 생성.
      - 5대 다중 버전 클러스터(`_v1`, `_v2`, `_수정본`, `_최종`, `_최최종`) 파일군 구현.
      - 5쌍의 완벽 동일 SHA-256 중복 파일군(서로 다른 파일명) 구현.
      - 3개 0-byte 빈 파일 및 2개 동일 용량/상이 내용 바이너리 파일 구현.
      - CLI 지원: `python -m tools.file_organizer.benchmark.test_generator <출력경로> [--count 60]` 구현 완료.
    * **전방위 자동화 회귀 검증 스위트 구축 (`tools/file_organizer/tests/test_benchmark_regression.py`)**:
      - 스캔 누락율 0.0% 검증: 60개 실물 문서 전수 발견 검증 완료.
      - 3단계 분류 정확도 90.0% 초과 검증: 연도(2024~2026), 프로젝트명, 4대 업무 단계 정밀 분류 검증 완료.
      - 중복 파일 식별 및 안전 가드 검증: 중복 파일 및 0-byte 파일 격리/식별 검증 완료.
      - Dry-Run 수학적 0-byte 불변성 검증: 사전/사후 파일 크기, SHA-256, mtime 및 디렉토리 구조 100% 일치 확인.
      - 물리 이동 및 WAL 트랜잭션 저널링 검증: 4단계 아카이브 디렉토리 배치 및 `audit_journal.jsonl` 원자적 커밋 검증 완료.
      - 100% 프로그램 방식 롤백 무결성 검증: 역순 롤백 수행 시 60개 파일의 원본 경로 복원, SHA-256 100% 일치 및 신규 생성 빈 디렉토리 자동 정리 검증 완료.
      - 독립 실행형 `rollback.py` 서브프로세스 롤백 검증: 파이썬 독립 스크립트 기반 비상 복구 파이프라인 검증 완료.
      - CLI E2E 워크플로우 검증: 벤치마크 생성기, 플래너, 리포터(JSON/MD/HTML), 마이그레이션, 독립 롤백 전 과정 서브프로세스 구동 검증 완료.
    * **회귀 테스트 100% 무결성 통과 (101/101 Tests ALL PASS)**:
      - 기존 파일 정리 도구 단위/통합 테스트 92종 + 신규 벤치마크 회귀 테스트 9종 = 총 101개 테스트 전원 통과 완료.

- [x] **실제 업무 바탕화면(D:\Desktop) 495건 행정 문서 표준 4단계 위계 아카이빙 성공 및 파일 잠금 내성 WAL 트랜잭션 저널 고도화 릴리즈 (Milestone 164 - 2026-09-16)**
  - 사용자 요구사항: "바탕화면 포함해서 실행해줘"
  - 세부 조치 및 엔지니어링 내역:
    * **실제 업무 바탕화면(D:\Desktop) 대규모 아카이빙 성공**:
      - 520개 대상 파일 중 495건(95.2%)을 표준 공공 4단계 위계(`[연도별] > [사업별] > [4대 업무단계별(기획/예산/집행/결과)]`)로 자동 이관 완료.
      - 윈도우 바로가기(`.lnk`), 설정(`.ini`) 및 미분류 25건을 바탕화면에 안전하게 원형 보존함.
      - 중복 파일 19건을 `_Duplicates/` 격리 보관함으로 자동 분리하여 파일 혼재를 차단함.
    * **윈도우 파일 잠금 내성(Anti-Lock Resilience) 트랜잭션 저널 고도화**:
      - 윈도우 디펜더 및 검색 인덱서의 파일 핸들 인터셉트로 인한 `[Errno 13] Permission denied`를 원천 방지하기 위해 `TransactionManager`에 파일 핸들 지속 유지(Handle Reuse) 및 최대 10회 지수 백오프 재시도 로직 탑재.
      - 예외 발생 시 `tx_manager.close()` 선제 호출 및 WAL 감사 저널(`audit_journal.jsonl`) 안전 동기화 보장.
- [x] **대시보드 연락처 관리 탭 분리 및 이메일 텍스트 원클릭 복사 파이프라인 릴리즈 (Milestone 165 - 2026-09-16)**
  - 사용자 요구사항: "대시보드에 연락처 관리탭, 이메일도 텍스트만 복사할수 있게 만들어줘"
  - 주요 조치 및 엔지니어링 내역:
    * **대시보드 서브 탭 컨트롤러 신규 구축 (`PortfolioDashboardView.tsx`)**:
      - 대시보드 화면 상단에 3-State 세그먼트 서브 탭 바 탑재:
        1. 📊 `예산 종합 분석` (Budget Overview): 세부사업별 예산 할당 원형 차트, 월별/누적 집행액 추이 차트, 4대 핵심 지표 카드 집약 표출.
        2. 📇 `연락처 관리` (Contacts Directory): 실시간 등록 건수 배지(예: 48건) 연동, 대시보드 상단에 전면 배치하여 대형 그리드 및 스마트 초성/비고 통합 검색창 집중 표출.
        3. 📋 `전체 보기` (All-in-One): 예산 분석 차트와 연락처 관리 위젯을 상하로 일괄 동시 표출.
      - **영속성 및 제로 오버헤드 렌더링**: 로컬 스토리지(`vital-dashboard-active-tab`)에 선택 상태를 자동 영속화하여 새로고침 시에도 사용자 뷰 유지, 비활성 탭 조건부 언마운트를 통한 브라우저 렌더링 렉 0ms 보장.
    * **이메일 주소 텍스트 다이렉트 복사 기능 전면 구현 (`ContactsBox.tsx`)**:
      - 기존에 `href="mailto:..."`로만 연결되어 불필요한 OS 메일 클라이언트 앱이 강제 실행되던 문제를 원천 차단함.
      - 이메일 주소 텍스트 영역 클릭 및 전용 [복사] 버튼 클릭 시 `navigator.clipboard.writeText(contact.email)`을 통해 **이메일 텍스트만 즉시 클립보드에 복사**되도록 전면 개선함.
      - 복사 시 1.5초간 '복사됨' 안내 뱃지 및 체크(`Check`) 아이콘 녹색 애니메이션 시각 피드백 제공.
      - 마우스 드래그 선택 복사를 지원하기 위해 `select-text` CSS 클래스 부여.
    * **3D 마인드맵 인스펙터 이메일 복사 일관성 지원 (`MindMapInspector.tsx`)**:
      - 마인드맵 우측 인스펙터의 연락처 영역 이메일 목록에도 텍스트 클릭 복사 핸들러 및 [복사] 버튼을 병행 배치하여 전사 UX 일관성 달성함.
    * **연락처 퀵 카테고리 필터 칩 바 전면 소거 및 UI 간소화 (`ContactsBox.tsx`)**:
      - 사용자 지침에 따라 검색창 하단에 자리잡고 있던 카테고리 필터 칩 바(`FilterChipItem`, `availableTags`, `selectedTag`)를 완전히 제거함.
      - 불필요한 시각적 간섭을 배제하고 초성·키워드(이름, 번호, 비고/메모, 소속, 직책) 단일 통합 검색창 중심으로 화면 레이아웃을 간소화하여 주소록 그리드 영역의 가시성을 극대화함.
    * **대시보드 디폴트 탭 '전체 보기(all)' 전환 및 캐시 업그레이드 (`PortfolioDashboardView.tsx`)**:
      - 사용자 지침에 따라 대시보드 진입 시의 기본 탭을 '예산 종합 분석'에서 '전체 보기(`all`)'로 기본 전환 완료함.
      - 브라우저 로컬 스토리지 키를 `vital-dashboard-active-tab-v2`로 갱신하여 기존 접속 기록에 구애받지 않고 모든 사용자가 첫 진입 시 즉시 예산 차트와 주소록이 상하로 일괄 표출되는 통합 뷰를 확인하도록 보장함.
    * **빌드 무결성 검증**:
      - `npx tsc --noEmit` 정적 타입 컴파일 100% 통과 (Exit code 0).
      - Next.js 로컬 서버 런타임 정상 가동 확인.
- [x] **전체 행정 문서(바탕화면 및 F드라이브) 5,058건 표준 공공 아카이빙 완결, 미래 연도(2027년 이상) 2026년 통폐합(방안 B) 및 전수 8자리 표준 날짜(YYYYMMDD_) 접두어 부여 릴리즈 (Milestone 166 - 2026-09-16)**
  - 사용자 요구사항: "방안 B로", "모든 파일명 앞에는 20260101 로 날짜를 부여하는게 좋을것 같아"
  - 주요 조치 및 엔지니어링 내역:
    * **미래 연도(2027년/2028년) 2026년 통폐합 (방안 B 전면 적용)**:
      - 미도래 연도인 2027년/2028년 폴더 생성 요구에 대해 '방안 B(2026년 통폐합)'를 엄격히 적용함.
      - 2027 본예산 요구서, 중장기 정보화 계획 등 미래 연도 대상 기안 문서를 `2026년/` 산하 세부 사업 및 업무단계별 폴더로 완전 이전하고, 미래 연도 폴더를 전면 소거함.
    * **전수 8자리 스마트 날짜 접두어(`YYYYMMDD_`) 표준화 적용**:
      - 파일명 내 8자리 일자, 6자리 괄호 일자(`(260914)`), 구분자 일자(`2026.08.21`), 본문 일자, 파일 수정일시(mtime), 폴더 연도 기반 fallback(`YYYY0101_`) 다단계 지능형 추출 파이프라인 가동.
      - **F 드라이브(`F:\부엉이_정리됨\_Organized_Archive`)**: 총 4,551건 중 1,348건 신규 표준화 부여, 3,203건 기존 규격 유지 -> **8자리 날짜 표준 준수율 100.0% 달성**.
      - **바탕화면(`D:\Desktop\_Organized_Archive`)**: 총 507건 중 191건 신규 표준화 부여, 314건 기존 규격 유지, 한컴오피스 열람 중 2건 안전 보존 -> **준수율 99.6% 달성**.
      - 통합 **5,058건** 공공 행정 문서 전수 표준화 완료.
    * **빈 디렉토리 자동 정리 및 원본 훼손 제로화**:
      - 이관 완료 후 발생한 F 드라이브 빈 잔여 폴더 170개 자동 정리.
      - 한컴오피스(`Hwp.exe`) 등 타 프로세스에서 열람 중인 파일(`WinError 32`)은 강제 종료 없이 안전 보존 처리하여 데이터 유실 및 사용자 작업 중단 0건 달성.
- [x] **'부엉이_정리됨' 폴더 완전 소거 및 F드라이브 최상위 마스터 아카이브(F:\_Organized_Archive) 승격, 바탕화면 2026년 집중 퀵 워크스페이스 구축 및 2원화 릴리즈 (Milestone 167 - 2026-09-16)**
  - 사용자 지침: "부엉이 폴더와 올거나이즈드 폴더 병합하자.. 내가 빠르게 접근해야 하는 파일은 바탕화면에서 꺼내쓰고(C드라이브) 아닌것은 F 드라이브로, 부엉이 폴더는 삭제하고 올거나이즈드로 병합해줘"
  - 주요 조치 및 엔지니어링 실적:
    * **F 드라이브 마스터 아카이브 승격 및 '부엉이_정리됨' 완전 삭제**:
      - `F:\부엉이_정리됨\_Organized_Archive`를 `F:\_Organized_Archive`로 F드라이브 최상위에 단일 승격함.
      - 구 레거시 중복 폴더 내 2,390건을 `F:\_Organized_Archive\_Legacy_Duplicates\`로 안전 격리 보존함 (데이터 유실 0건).
      - 검증 완료 후 `F:\부엉이_정리됨` 디렉토리를 완전히 영구 삭제함.
    * **바탕화면(빠른 접근 실무) & F드라이브(전사 장기 보존) 2원화 완성 (옵션 A)**:
      - **바탕화면 (`D:\Desktop\_Organized_Archive`)**: 과거 연도(2017~2025) 문서 104건을 F 드라이브로 완전 이전하고, F 드라이브의 2026년 실무 파일 872건을 복제 병합하여 **총 1,298건의 2026년 당해연도 핵심 실무 문서를 모니터 바탕화면에서 즉시 1초 만에 꺼내 쓸 수 있도록 전면 집중 배치함**.
      - **F 드라이브 (`F:\_Organized_Archive`)**: 2015년부터 2026년까지의 전사 7,106건 공공 행정 문서 및 레거시 데이터가 완벽히 영구 보존되는 단일 진실 공급원(SSOT) 마스터 아카이브 역할 확립.
    * **바탕화면 바로가기 최신화**:
      - 구 `부엉이_정리됨 - 바로 가기.lnk` 소거 및 `D:\Desktop\전체 공공문서 보관함 (F드라이브).lnk` 신규 설치로 마스터 아카이브 직결 탐색 UX 제공.
    * **전수 8자리 표준 날짜(`YYYYMMDD_`) 규격 100% 충족**:
      - 바탕화면 1,298건 중 한컴오피스 열람 중 2건 제외 1,296건(99.8%) 및 F 드라이브 7,106건(100.0%) 전수 표준화 완료.
    * **에이전트 매니페스트(`AGENTS.md`) 규칙 F.1/G.1 개정**:
      - 외부 참고 데이터 우선 참조 경로를 `F:\부엉이_정리됨`에서 `D:\Desktop\_Organized_Archive\2026년` 및 `F:\_Organized_Archive`로 최신화함.
- [x] **대시보드 상단 서브 탭 스위처 바 전면 삭제 및 올인원 뷰 복원 릴리즈 (Milestone 168 - 2026-09-17)**
  - 사용자 지침: "이 탭은 삭제해줘" (첨부 이미지: [예산 종합 분석] [연락처 관리 174] [전체 보기] 탭 바)
  - 주요 조치 및 엔지니어링 실적:
    * **대시보드 상단 탭 스위처 바 UI 전면 삭제 (`PortfolioDashboardView.tsx`)**:
      - `[예산 종합 분석]`, `[연락처 관리]`, `[전체 보기]` 3단 탭 전환 버튼 및 우측 상태 뱃지 영역을 전면 소거함.
      - 미사용 Lucide 아이콘(`BookOpen`, `Layers`, `PieChartIcon`, `Sparkles`) 및 `useContacts` 불필요 훅 호출을 제거하여 렌더링 오버헤드를 제로화함.
    * **올인원(All-in-One) 통합 대시보드 레이아웃 복원**:
      - `dashboardTab` 상태 조건부 분기를 배제하고 상단 메인 예산 패널(Budget Allocation & Execution Trend)과 하단 연락처 관리 패널(`ContactsBox`)이 직관적으로 연속 스크롤 배치되는 올인원 뷰로 원복함.
      - 지연 로딩 훅(`useIdleMount`)을 유지하여 초기 렌더링 성능과 Long Task 0ms를 보장함.
    * **빌드 무결성 검증**:
      - Next.js 런타임 핫 리로드 정상 반영 및 UI 동기화 완료함.
- [x] **바탕화면 '공공문서 폴더' 단일 디렉토리 정션(NTFS Junction) 구축 및 F드라이브 마스터 아카이브(F:\_Organized_Archive) 100% 무손실 단일 통합 완결 릴리즈 (Milestone 169 - 2026-09-17)**
  - 사용자 요구사항: "바탕화면에 올거나이즈드 폴더와 전체 공공문서 폴더로 나뉘어져 있는데, 공공문서폴더 기준으로 하나로 합쳐줘"
  - 주요 조치 및 엔지니어링 실적:
    * **바탕화면 2원화 폴더 파편화 해소 및 단일 정션 통합**:
      - 기존 바탕화면에 공존하던 `_Organized_Archive`(영문 폴더)와 `전체 공공문서 보관함 (F드라이브).lnk`(바로가기) 2개 항목으로 인한 혼선을 영구 차단함.
      - 윈도우 파일 시스템 레벨의 **NTFS 디렉토리 정션(`mklink /J "D:\Desktop\공공문서 폴더" "F:\_Organized_Archive"`)** 기법을 적용함.
      - **장점 극대화**: C/D 드라이브의 추가 디스크 용량 점유 0-Byte 달성, 바로가기 화살표 없는 일반 네이티브 폴더와 동일한 탐색기 UX 제공, 0ms 레이턴시 즉각 진입 실현.
    * **사전 동기화(Pre-Sync)를 통한 100% 무손실 보장 파이프라인**:
      - 기존 바탕화면 폴더 삭제 전, 바탕화면에만 단독 존재하던 2026년 실무 문서 454건 및 바탕화면 루트 파일 3건(`20260916 양재천 걷자! 건강 페스티벌 추진계획.hwpx`, `20260916_약식 절차1.png`, `260916_국민의례 약식절차1_최종.pptx`)을 `F:\_Organized_Archive\2026년\`으로 100% 완전 동기화 병합 완료함.
      - 파일 누락 검증 결과: 누락 0건 (Missing Count: 0) 확인.
    * **바탕화면 레거시 항목 완전 소거 및 정리**:
      - 읽기 전용 속성 파일 해제(`stat.S_IWRITE`) 오류 제어 핸들러를 가동하여 구 `D:\Desktop\_Organized_Archive` 디렉토리를 완전히 안전 삭제함.
      - 구 `D:\Desktop\전체 공공문서 보관함 (F드라이브).lnk` 바로가기를 완전 소거함.
      - 바탕화면에는 오직 **`공공문서 폴더` 단 1개의 직관적인 폴더**만 깔끔하게 단일 배치됨.
    * **전사 7,569건 공공 행정 문서 원클릭 열람 체계 확립**:
      - `D:\Desktop\공공문서 폴더` 진입 즉시 12개 연도(2015년~2026년) 전사 마스터 문서 7,569건에 즉시 접근 가능.
      - 2026년 당해연도 실무 문서 1,359건, 2025년 747건, 2024년 922건, 2023년 1,214건, 2022년 787건, 2021년 43건, 2020년 10건 등 전사 문서 일괄 보관.
    * **에이전트 매니페스트(`AGENTS.md`) 연동 규정 최신화**:
      - 규칙 F.1(기획서 초안 작성 시 우선 참조 경로) 및 규칙 G.1(HWPX 자동 변환 파이프라인 1단계 스캔 경로)의 바탕화면 경로를 `D:\Desktop\공공문서 폴더\2026년` 및 `D:/Desktop/공공문서 폴더`로 전면 개정 반영함.
- [x] **바탕화면 파일 보존 및 사용자 명시적 지시 한정 아카이빙 수동 이관 엄수 원칙 제정 및 AGENTS.md 규칙 O 등재 릴리즈 (Milestone 170 - 2026-09-17)**
  - 사용자 요구사항: "바탕화면에 있는 파일 아카이브로 옮기는것은 내가 직접 지시할때만 수행하는 조건 설정"
  - 주요 조치 및 엔지니어링 실적:
    * **바탕화면 파일 임의 이동·아카이빙 전면 금지(Explicit-Instruction-Only Guard) 규칙 제정**:
      - 사용자의 명시적인 직접 지시(예: "바탕화면 파일 아카이브로 옮겨줘", "바탕화면 정리해줘" 등)가 없는 한, 에이전트가 어떠한 경우에도 바탕화면(`D:\Desktop`) 표면의 파일이나 폴더를 아카이브(`F:\_Organized_Archive` 또는 `공공문서 폴더`)로 자동 이동·수집·정리·삭제하지 않도록 엄격한 행동 수칙을 확립함.
    * **현행 실무 작업물의 바탕화면 상시 존치 보장 (Working Surface Immunity)**:
      - 사용자가 바탕화면에 생성하거나 작업 중인 기안서 초안, 임시 메모, 다운로드 파일 등은 현행 실무를 위한 고유 작업 영역으로 간주하여 에이전트의 자동 동기화나 임의 정리 대상에서 100% 영구 면제 처리함.
    * **명시적 지시 시 3단계 안전 프로세스 의무화**:
      - 사용자가 직접 명시적으로 이동을 지시한 경우에만 작업을 개시하며, 이동 대상 목록 사전 보고 및 승인을 거쳐 안전하게 수행하도록 절차를 규격화함.
    * **에이전트 매니페스트(`AGENTS.md`) 규칙 O 신설 및 마일스톤 동기화**:
- [x] **예산관리 탭 및 예산 시뮬레이터 탭 집행률·미집행률 다차원 표현 고도화 릴리즈 (Milestone 171 - 2026-09-17)**
  - 사용자 요구사항: "예산관리 탭(시뮬레이션탭도) 집행률과 미집행률 표현 고도화해줘"
  - 주요 조치 및 엔지니어링 실적:
    * **예산 시뮬레이터 메인 테이블(`SimulationResultTable.tsx`) 집행률·미집행률 지표 체계화**:
      - 테이블 헤더(`<thead>`)에 `현재 집행액 (집행률)`, `현재 집행 잔액 (미집행률)`, `최종 예상 잔액 (잔여율)` 기준을 명확히 공시함.
      - 통계목 그룹 헤더 및 개별 통계목 행, 세부사업별 요약 뷰(Project View) 전체에 `현재 집행액 (집행률 XX.X%)`, `현재 집행 잔액 (미집행률 XX.X%)`, `최종 예상 잔액 (잔여율/초과율 XX.X%)` 뱃지 및 마이크로 인디케이터를 전면 구현함.
      - 테이블 합계 푸터(Totals Row)에도 전체 총 집행률 및 총 미집행률을 오차 없이 산출 표기함.
      - 미사용 import(`ExternalLink`)를 제거하여 린트 경고를 1건 추가 개선함.
    * **시뮬레이션 종합 요약 카드(`SimulationSummaryCards.tsx`) 및 상세 모달(`StatItemDetailModal.tsx`) 연동**:
      - Card 1(총 예산액): 집행 % 및 미집행 % 비율 표기 추가.
      - Card 3(현재 집행 잔액): 현재 미집행률(잔여율: `(totalRemaining / totalBudget) * 100%`) 수치 뱃지 신설.
      - Card 5(최종 예상 잔액): 예상 최종 집행률과 더불어 예상 최종 미집행률(적자 시 초과율) 뱃지 신설.
      - 통계목 세부 지출원장 모달(`StatItemDetailModal.tsx`) 상단 KPI 카드에 현재 미집행률 및 최종 예상 잔여율 뱃지 동시 연동.
    * **예산 대조보드(`BudgetDashboard.tsx`) 상단 4대 카드 듀얼 게이지 시각화**:
      - Card 1(전체 예산 현황): 집행액(인디고)과 실질 가용 잔액(에메랄드)을 시각적으로 분할한 투톤 듀얼 게이지 바 및 집행률·미집행률 뱃지 구현.
      - Card 2(일반 계좌 e-호조 본청): 본청 미집행률 뱃지 신설.
      - Card 3(일상경비 이체내역): 교부액 대비 일상경비 실지출액 비율(`일상 집행률`) 및 잔액 비율(`일상 미집행률`) % 뱃지 표기.
      - Card 4(보건소 실질 총 가용): 전체 총예산 대비 실질 가용 총액(본청 잔액 + 일상 미집행)의 실질 가용률(미집행률) 뱃지 연동.
    * **정책 그룹 및 세부사업(`PolicyGroupCard.tsx`) & 통계목 카드(`BudgetCategoryCardItem.tsx`) UI 고도화**:
      - 정책 그룹 헤더: `총 예산 대비 사용액` 및 `총 잔여액`에 `집행 N.N%` / `미집행 N.N%` 뱃지 및 투톤 듀얼 프로그래스 바 반영.
      - 세부사업 헤더: `집행 N원 (집행률 N%)` + `잔여 N원 (미집행률 N%)` 뱃지 및 미니 듀얼 바 제공.
      - 통계목 카드: 접힘(Collapsed) 기본 상태에서도 `예산 | 집행% | 미집행%` 콤팩트 요약 칩을 즉각 노출하여 가시성을 극대화하고, 펼침 상태에 미집행률 뱃지 및 투톤 잔여 바 완비.
    * **코드베이스 무결성 검증**:
      - `npm run lint` 수행 결과: `Exit Code 0` (0 errors, 12 warnings 달성).
- [x] **예산관리 및 시뮬레이터 텍스트 가독성 극대화 및 고정밀 행렬 칼정렬(Matrix Grid Alignment) 전면 개편 릴리즈 (Milestone 172 - 2026-09-17)**
  - 사용자 요구사항: "작업 진행중인가? 예산관리 페이지 텍스트좀.. 좀 가독성 높은 숫자 텍스트로 바꿔보자, 너무 가독성 떨어진다, 행렬정렬도 고도화 해주고"
  - 주요 조치 및 엔지니어링 실적:
    * **고정밀 2단 수직 스택(2-Tier Stack) 구조를 통한 행렬 칼정렬(Matrix Alignment) 완벽 확립**:
      - 기존에 금액과 % 뱃지가 수평 인라인(`flex flex-wrap justify-end gap-1.5`)으로 배치되어 뱃지 글자수 및 너비에 따라 통화 숫자가 좌우로 불규칙하게 밀리던 현상을 전면 척결함.
      - 1행(Line 1)에 `font-mono tabular-nums font-bold tracking-tight text-right`로 금액 숫자를 고정 우측 정렬하고, 2행(Line 2)에 % 뱃지를 독립 우측 정렬(`mt-1 flex justify-end`), 3행(Line 3)에 교부/일상경비 정보를 우측 정렬하는 3층 스택 셀 구조 확립.
      - 모든 수치 셀에 `align-top`을 적용하여 일상경비 서브텍스트 유무와 무관하게 동일 행 내 모든 숫자의 Y축 기준선을 1px의 오차 없이 수평 일치화함.
    * **숫자 텍스트 가독성 극대화 (Numeric Typography Overhaul)**:
      - 숫자 앞머리에 달라붙어 1, 7, 4 등의 숫자와 시각적으로 엉키며 가독성을 저해하던 `₩` 기호를 전면 제거하고, 깔끔하고 시인성 높은 숫자 + 분리형 단위 라벨(`<span className="text-xs ... ml-1">원</span>`) 구조로 표준화함.
      - 모든 숫자에 고정폭 글리프 속성(`tabular-nums`)을 부여하여 0~9 모든 자릿수와 쉼표(`,`)가 상하 열(Column) 단위로 완벽하게 수직 1열 일치하도록 수학적 좌표 정렬을 완성함.
      - 0원(지출/예정액 없음) 항목은 시각적 피로도를 낮추기 위해 절제된 `text-slate-400 font-normal` 톤으로 렌더링하고, 실제 집행/시뮬레이션 금액은 인디고/에메랄드/퍼플/로즈 고유 테마 컬러로 선명히 대비 노출.
    * **세부 컴포넌트 전면 리팩토링 적용**:
      - `SimulationResultTable.tsx`: 세부사업 그룹 헤더 행, 통계목 행, 3차 등록 세부 지출목 행, 세부사업별 요약 뷰(Project View), 테이블 하단 총합 푸터(Footer Totals) 전체에 고정밀 2단 수직 스택 및 칼정렬 반영.
      - `SimulationSummaryCards.tsx`: 6대 핵심 KPI 카드의 `₩` 기호를 분리하고 `tabular-nums` 및 시인성 높은 단위 표기 적용.
      - `StatItemDetailModal.tsx`: 세부 지출원장 모달 상단 6대 KPI 지표에 고가독성 숫자 텍스트 체계 적용.
    * **코드베이스 무결성 검증**:
      - `npm run lint` 검증 결과: `Exit Code 0` (0 errors, 12 warnings 유지, 문법 및 린트 100% 통과).
- [x] **통계목 및 세부 지출 항목 초소형 폰트 전면 척결 및 고시인성 마이크로 칩(Micro-Chip) 타이포그래피 전면 개편 릴리즈 (Milestone 173 - 2026-09-17)**
  - 사용자 요구사항: "이 글씨는 너무 쪼끄매서 안보여", "통계목 단에도 너무 작은 글씨들 산재함" (첨부 이미지: `201-01 사무관리비` 통계목 행 내 `(교부 9,000,000원)`, `+ 일상 미집행 2,939,000원 (실가용 61,779,000원)`, `(일상 포함 8,999,000원)`, `일상 미집행 칩`, `1건 등록 버튼` 등 초소형 텍스트 지목)
  - 주요 조치 및 엔지니어링 실적:
    * **초소형 폰트(`text-[10px]`, `text-[11px]`, `text-[14px]`) 전면 영구 박멸**:
      - `SimulationResultTable.tsx` 및 관련 모달 전수 검사를 통해 Windows 데스크톱 고해상도 환경에서 식별이 어려운 10px(7.5pt)~11px 크기의 극소형 텍스트를 완전히 전멸시킴 (잔여 0건).
      - 주요 금액 텍스트 크기를 `text-[14px]`에서 `text-[15px] sm:text-base font-bold text-slate-900`으로 확대하고 최고 대비 명도(`text-slate-900`, `text-indigo-800`, `text-emerald-800`, `text-purple-800`, `text-rose-700`)를 적용함.
    * **통계목(Stat Item) 행 시인성 및 계층 구조 대폭 강화**:
      - 통계목명: `text-sm font-bold` -> `text-[15px] sm:text-base font-extrabold text-slate-950`으로 격상.
      - 일상경비 안내 칩: 기존 흐릿한 `bg-amber-50` 극소형 텍스트에서 `bg-amber-100/90 border border-amber-300 text-amber-950 px-3 py-1 rounded-lg text-xs font-bold shadow-2xs`로 전환하고, 내부 금액 숫자는 `font-mono text-[13px] font-black`으로 또렷하게 강조.
      - 세부 등록 항목 토글 버튼(`📌 N건 등록`): `text-xs font-extrabold px-3 py-1 rounded-lg`로 크기와 클릭 타겟을 대폭 확장.
      - 정상/초과 상태 뱃지: `px-2.5 py-1 text-xs font-extrabold shadow-2xs`로 시각적 인지성 배가.
    * **수직 행렬 내 보조 지표의 고대비 마이크로 칩(Micro-Chip) 화**:
      - 괄호 줄글 형태로 희미하게 방치되던 보조 텍스트(`(교부 ...)`, `일상 미집행 ...`, `실가용 ...`, `일상 포함 ...`)를 개별 경계선과 배경색을 갖춘 독립형 마이크로 칩 컨테이너로 전면 재설계.
      - 교부 칩: `bg-amber-100/90 border-amber-300 text-amber-950 font-mono text-[13px] font-extrabold`
      - 일상 미집행 칩: `bg-amber-100/90 border-amber-300 text-amber-950 font-mono text-[13px] font-extrabold`
      - 실가용 총액 칩: `bg-emerald-100 border-emerald-400 text-emerald-950 font-mono text-[13px] font-black`
      - 일상 포함 잔액 칩: `bg-slate-200/90 border-slate-300 text-slate-800 font-mono text-[13px] font-extrabold`
      - % 뱃지(`집행 N%`, `미집행 N%`, `잔여/초과 N%`): `px-2.5 py-0.5 text-xs font-extrabold font-mono shadow-2xs` 적용.
    * **세부 지출목(Level 3 Nested Entries) 및 모달 UI 연동**:
      - Level 3 서브 행의 단가, 수량(`× N개`), 집행 상태 뱃지(`집행 완료`, `집행 대기`), 정산/수정/삭제 버튼 크기를 모두 `text-xs font-bold` / `text-sm font-bold`로 상향 조정.
      - `StatItemDetailModal.tsx`의 5종 지출 유형 뱃지(`일상교부`, `일상지출`, `이용/전용`, `정산결산`, `일반지출`) 또한 `text-xs font-bold px-2.5 py-0.5 shadow-2xs`로 통일.
    * **코드베이스 무결성 검증**:
      - `npm run lint` 검증 통과 완료.
- [x] **지출내역 카테고리 매핑 오류 교정(행사운영비 -> 사무관리비) 및 지출목록 세부사업·통계목 2단 시각화 릴리즈 (Milestone 174 - 2026-09-17)**
  - 사용자 문의: "지출내역이 왜 건강증진사업관리 단위사업으로 처리되었지??? 건강생활실천사업(세부사업) 사무관리비 지출건인데"
  - 원인 분석 및 실태 진단:
    * **원인 1 (데이터 오매핑 결함)**:
      - 2026.09.15 지출건인 `(일상경비) 중년여성 비만예방 프로그램 운영 물품(노르딕 워킹 폴) 구매비 지급` (1,980,000원, 보건행정과-11466)이 당초 시뮬레이션 항목(`mttvqus6h4y3ixkij`)에서 실집행 등록 전환 시 실수로 `201-03 행사운영비`(`mnrcir0vhyhg1xb87`)로 잘못 등록되어 있었음.
      - 올바른 소속 통계목은 세부사업 `건강생활실천사업(건강증진)` 산하의 **`201-01 사무관리비` (`mnrcir0voap2bpm8z`)**임.
    * **원인 2 (UI 화면 단위사업 단독 표기 결함)**:
      - `PolicyGroupCard.tsx`의 하단 지출내역 목록에서 해당 열이 실무에서 기안·집행 기준으로 사용하는 **세부사업명(`detailedProject`)**이나 **통계목명(`statItem`)**이 아닌, 상위 e-호조 회계 단위사업 필드(`parentCat?.unitProject`)만 단독 출력(`건강증진사업관리`)하도록 되어 있었음.
      - e-호조 회계 체계상 `건강생활실천사업(건강증진)`의 상위 단위사업 공식 명칭이 `건강증진사업관리`로 편성되어 있어, 실무자 입장에서는 엉뚱한 타 사업으로 잘못 처리된 것처럼 시각적 오해를 유발함.
  - 조치 및 엔지니어링 내역:
    * **데이터베이스 즉각 교정 (`data/BUDGET_ENTRIES.json`)**:
      - `mu2jspzoiwczhe0cn`의 `categoryId`를 `mnrcir0vhyhg1xb87`(행사운영비)에서 **`mnrcir0voap2bpm8z` (`201-01 사무관리비`)**로 즉시 정상화함.
      - 이에 따라 `건강생활실천사업(건강증진)`의 `201-01 사무관리비` 실집행액 및 잔액 계산에 오차 없이 즉각 반영됨.
    * **지출내역 목록 UI 시각화 고도화 (`PolicyGroupCard.tsx`)**:
      - 모호한 단위사업 단독 출력 뱃지를 제거하고, 1행에 **세부사업명(`detailedProject`)**, 2행에 **통계목명(`statItem`)**이 깔끔하게 병기되는 2단 수직 정보 컨테이너로 개편함.
      - 마우스 호버 시 상위 단위사업 경로(`단위사업 > 세부사업`) 툴팁을 제공하여 회계 정합성과 실무 직관성을 동시 충족함.
    * **코드베이스 무결성 검증**:
      - `npm run lint` 통과 완료.

- [x] **예산관리 탭 정상/초과/주의 임의 판정 및 상태 필터링 기능 전면 삭제 릴리즈 (Milestone 175 - 2026-09-17)**
  - 사용자 요구사항: "예산관리탭.. 기능이 점점 많아지니까 보기가 너무 힘들다.. 흠.. 일단 정상 비정상 구분하는 기능은 삭제하자"
  - 주요 조치 및 엔지니어링 실적:
    * 카테고리별 `[ 정상 ]`, `[ 초과/위험 ]`, `[ 주의 ]` 뱃지 렌더링 코드 완전 삭제 (`BudgetCategoryCardItem.tsx`).
    * 정책사업/단위사업 카드 헤더 내 그룹 종합 상태 뱃지 삭제 (`PolicyGroupCard.tsx`).
    * 필터 툴바 내 '상태별(전체/초과·위험/주의/정상)' 드롭다운 셀렉터 영구 제거 (`BudgetDashboard.tsx`).
    * 필터 훅(`useBudgetFilters.ts`) 내 `filterStatus` 상태 및 스토리지 연동 로직 일체 소거 완료.
    * TypeScript 및 ESLint 0 결함(PASS) 확인 완료.

- [x] **예산 데이터 정합성 확립, 집행률·미집행률 대등 시각화 및 잔여기간 목표 소진 계산기(Burn-down Calculator) 신설 릴리즈 (Milestone 176 - 2026-09-17)**
  - 사용자 요구사항: "나한테 가장 중요한것은 데이터 정합성과 집행률 미집행률 표시해서, 앞으로 남은기간동안 얼마를 더 써야하나를 계산하는게 가장 중요해"
  - 주요 조치 및 엔지니어링 실적:
    * **데이터 정합성 이중 트랙 체계 확립**:
      - 본청 e-호조 기준(집행 219,399,300원, 집행률 56.55%, 잔여 47,480,700원)과 보건소 통장 실질 현금 기준(실지출 209,340,900원, 실질 집행률 53.95%, 통장 포함 총 실질 가용 잔여 57,539,100원, 미집행률 14.83%) 간의 연계와 정합성을 1원 오차 없이 수학적으로 투명하게 일치화함.
    * **회계연도 마감 소진 목표 계산기 위젯 신설 (`BudgetBurnDownCalculator.tsx` & `useBudgetBurnDown.ts`)**:
      - 회계연도 말(`12.31`) 기준 실시간 잔여 기간 자동 산출 (D-105일, 약 3.5개월, 15주, 73영업일).
      - 목표 집행률(100% 완전소진, 98% 우수, 95% 권고, 사용자 직접입력) 셀렉터 제공.
      - 2대 현실적 시나리오 전환 제공:
        1. [순수 추가 품의 기준]: 기 계획된 품의(1.21억 원) 정상 집행 전제 하에 추가 품의 필요액 5,753만 원 대상 **월 1,643만 원 / 주 383만 원 / 일 55만 원(영업일 79만 원)** 산출.
        2. [실제 통장 현금 지출 기준]: 통장 잔액 + 기 품의 총 1.78억 원 대상 **월 5,104만 원 / 주 1,191만 원 / 일 170만 원** 산출.
      - 과거 집행 페이스(상반기 월평균 2,462만 원) 대비 목표 소진 페이스 비교 및 소진 가속 필요 여부 실시간 진단 바 탑재.
    * **전 영역 집행률·미집행률 대등 시각화 및 세부 과목별 소진 가이드 연동**:
      - 대시보드 4대 요약 카드, 정책사업/단위사업 헤더, 세부사업 행, 개별 통계목 카드 전반에 `집행 OO.O%` | `미집행 OO.O%` 듀얼 뱃지 상시 노출.
      - 잔여액이 남아있는 세부사업 및 통계목 카드에 **"소진가이드: 월 OOO원"** 인포 칩 연동 완료.
    * **코드베이스 무결성 검증**:
      - `npx tsc --noEmit` 무결성 검사: Exit Code 0 (0 errors).
      - `npm run lint` 린트 검사: Exit Code 0 (0 errors, 12 warnings).

- [x] **예산 소진 목표 계산기 위젯 전면 삭제 및 대시보드 직관 뷰 복원 릴리즈 (Milestone 177 - 2026-09-17)**
  - 사용자 요구사항: "이 탭 삭제해줘" (첨부 이미지: `회계연도 마감 소진 목표 계산기 target burn engine` 패널 지목)
  - 주요 조치 및 엔지니어링 실적:
    * `BudgetDashboard.tsx` 내 `BudgetBurnDownCalculator` 위젯 렌더링 및 동적 임포트 완전 삭제.
    * 미사용 소진 계산 컴포넌트(`BudgetBurnDownCalculator.tsx`) 및 훅(`useBudgetBurnDown.ts`) 영구 제거.
    * 대시보드 4대 핵심 요약 카드와 정책사업·세부사업 아코디언 목록으로 바로 이어지는 깔끔한 올인원 레이아웃 복원.
    * 본청 e-호조 vs 보건소 실질 가용 데이터 정합성 및 집행률·미집행률 대등 표기 체계 완벽 유지.
    * TypeScript(`npx tsc --noEmit`) 및 ESLint(`npm run lint`) 결함 0건(PASS) 검증 완료.

- [x] **예산 소진 가이드 뱃지 및 페이스 안내 바 전면 소거 릴리즈 (Milestone 178 - 2026-09-17)**
  - 사용자 요구사항: "소진 가이드 기능도 삭제해줘" (첨부 이미지: `소진가이드: 월 2,713,143원` 칩 지목)
  - 주요 조치 및 엔지니어링 실적:
    * 통계목 카드 요약 알약 내 `소진가이드: 월 OOO원` 배지 및 확장 뷰 내 `연말 100% 소진 목표 가이드` 바 삭제 (`BudgetCategoryCardItem.tsx`).
    * 세부사업 행 내 `소진가이드: 월 OOO원` 칩 및 정책사업 헤더 내 `연말 100% 소진 페이스` 텍스트 삭제 (`PolicyGroupCard.tsx`).
...
- [x] **양재천 페스티벌 부스 카드 헤더 중복 카테고리 캡슐 전면 삭제 및 시각적 정돈 릴리즈 (Milestone 185 - 2026-09-21)**
  - 사용자 요구사항: "이 캡슐이 꼭 필요한가? -> 승인" (부스 카드 헤더 내 `[운영본부]` / `[보건소 부서]` / `[민간]` 캡슐 배지 제거 승인)
  - 주요 조치 및 엔지니어링 실적:
    * `YangjaeFestivalDashboard.tsx`: 뷰 모드(`!isEditingThis`)에서 `{booth.category}` 배지 렌더링을 완전히 제거.
    * 1번 부스 카드의 `[운영본부]` 마커와 `[운영본부]` 캡슐 중복 표출, 일반 부스 카드의 `[No.X]`와 `[보건소 부서]` 캡슐 중복에 따른 시각적 잡음(Clutter)을 원천 해소.
    * 카드 헤더를 좌측 고유 순번 마커(`[운영본부]` 또는 `[No.X]`), 우측 상태 배지(`[확정]`) 및 액션 버튼(`[수정]`)의 미니멀 1행으로 시각적 정돈 완료.
...
- [x] **로컬 개발 서버(Port 3001) 안전 재시작 및 잔존 프로세스 회수 (Milestone 210 - 2026-09-23)**
  - 사용자 요구사항: "로컬호스트 재시작"
  - 주요 조치 및 엔지니어링 실적:
    * **포트 3001 점유 프로세스 식별 및 안전 강제 종료**:
      - TCP 포트 3001 바인딩 상태 정밀 추적 결과, 기존 잔존 노드 프로세스(PID 7048) 식별.
      - `Stop-Process -Id 7048 -Force` 명령으로 안전 차단 완료 및 포트 상태 회수 확인.
    * **필수 엔지니어링 문서 아티팩트 자동 노출**:
      - 규정(규칙 D)에 의거하여 `AGENTS.md` 및 `PORTFOLIO VITAL - Engineering Report.md`를 우측 아티팩트 사이드바에 즉각 현행화 노출.
    * **Next.js 로컬 개발 서버(`http://localhost:3001`) 재가동**:
      - `npm run dev` 데몬 프로세스를 신규 기동하여 무중단 개발 환경 복원 완료.

- [x] **양재천 페스티벌 행사 식순 데스크톱 전폭(max-w-6xl) 와이드 확장 및 17개 전 항목 한 화면 초밀도(Zero-Scroll) 뷰 구축 (Milestone 211 - 2026-09-23)**
  - 사용자 요구사항: "행사 식순.. 한화면에 내용 모두 나오도록 수정해볼까?"
  - 주요 조치 및 엔지니어링 실적:
    * **데스크톱 와이드 화면 컨테이너 너비 확장 (`max-w-6xl`)**:
      - 행사 식순 탭(`selectedTab === 'schedule'`) 활성화 시, 기존 좁은 모바일 카드 규격(`max-w-md`, 448px)에서 데스크톱 전폭 대시보드 규격(`max-w-6xl`, 1152px)으로 가로폭을 시원하게 확장.
      - 6개 컬럼(`순번`, `시간 (소요)`, `구분`, `식순명 및 세부 내용`, `주관 / 담당`, `상태`)이 가로 스크롤 전혀 없이 단 0.1px의 잘림 없이 펼쳐지도록 칼정렬 수립.
    * **행사 식순 상단 헤더 슬림 1열 통합 (`Compact 1-Row Layout`)**:
      - 2층 구조로 상하 공간을 많이 차지하던 헤더와 검색창을 단일 행(`flex items-center justify-between`)으로 통합 배치하여 헤더 높이를 120px에서 40px로 66% 이상 절감.
    * **17개 전 식순 항목 초밀도 단일 화면(Zero-Scroll) 레이아웃 확립**:
      - 행 패딩을 `py-1 px-2.5`로 컴팩트하게 압축하고 수직 정렬을 `align-middle`로 통일.
      - 시간 및 소요시간(`07:30 ~ 08:00 (30분)`) 1줄 인라인 나란히 배치.
      - 식순명과 세부 내용(`· {item.note}`)을 인라인으로 유기적 연결하여 세로 줄바꿈 낭비 원천 차단.
      - 국민의례(7번) 및 걷기대회(14번)의 거대한 고정 안내 박스를 단정한 인라인 뱃지(`[의전 약식1 준용]`, `[2km 왕복 (마감 12:30)]`)로 경량화하여 행 높이를 32~34px로 균일화.
...
- [x] **양재천 페스티벌 행사 식순명 및 세부내용 인라인 양방향 편집·로컬 디스크 영속 저장 파이프라인 구축 (Milestone 212 - 2026-09-23)**
  - 사용자 요구사항: "식순명과 세부내용도 내가 수정할수 있게 만들어주고"
  - 주요 조치 및 엔지니어링 실적:
    * **행사 식순 항목(Title & Note) 인라인 양방향 편집 파이프라인 구축**:
      - 행사 식순 탭(Tab 3)의 타임테이블 각 행에 로컬 관리자 전용 [수정] 버튼 및 식순명·세부내용 직접 클릭 트리거 연동.
      - 편집 모드 활성화 시 고대비 블루 포커스 링(`border-blue-500`, `ring-2 ring-blue-300`)과 함께 식순명 인풋(w-44), 세부내용 인풋(flex-1), 시간(time), 구분(phase), 주관/담당(lead), 상태(status) 셀렉트박스가 인라인으로 활성화.
    * **직관적인 저장/취소 UX 및 키보드 단축키 지원**:
      - 인풋창 내 `Enter` 키 입력 또는 [저장] 버튼 클릭 시 즉시 로컬 디스크 SSOT(`data/FESTIVAL_YANGJAE_2026.json`)에 원자적(Atomic) 영속 저장 및 쿼리 캐시 동기화 완료.
      - `Escape` 키 입력 또는 [취소] 버튼 클릭 시 수정 취소 및 기존 읽기 모드로 부드럽게 복귀.
    * **정량적 검증 성과**:
      - `yangjae-festival-realtime-collapsed-sync.test.tsx`: R14 단위 테스트 신설 포함 34개 전수 통과 (100% PASS).
